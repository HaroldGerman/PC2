...
@test "Normaliza y deduplica registros en CSV" {
  ...
  # Debe haber solo dos líneas (deduplicado)
  [ "$(wc -l < ../out/normalized.csv)" -eq 2 ]
  # Verificar minúsculas y formato CSV
  grep 'primer log simulado' ../out/normalized.csv
  grep 'segundo log simulado' ../out/normalized.csv
}
